...
COLORS = {
    ...
    "torch compile (reduce-overhead)": "#c070e0",
...
MARKERS = {
    ...
    "torch vmap (eager)": "o",
    "torch compile": "v",
    "torch compile (reduce-overhead)": "d",
    "torch compile (H4)": "^",
    "torch compile (H4+H8)": "^",
    "MJX jit(vmap)": "P",
}
...
LABELS = {
    ...
    "torch compile (reduce-overhead)": "mujoco-torch compile (reduce-overhead)",
...
def plot_model(ax, model_name, model_data, all_batch_sizes):
    ...
    for config_name, config_data in model_data.items():
        ...
        if len(bs) == 1 and bs[0] == 1:
            ...
            ax.plot(bs, sps, marker=marker, color=color, markersize=7, label=label, linestyle="none", zorder=5)
        else:
            ax.plot(bs, sps, marker=marker, color=color, markersize=7, linewidth=2, label=label, zorder=5)
...
def main():
    ...
    parser.add_argument(
        "--output", "-o", default="assets/benchmark.png", help="Output image path (default: assets/benchmark.png)"
    )
    ...
    fig.suptitle("mujoco-torch GPU Benchmark (H200, float64, 1000 steps)", fontsize=15, fontweight="bold", y=1.02)
    ...
    fig.savefig(args.output, dpi=args.dpi, bbox_inches="tight", facecolor="white", edgecolor="none")
