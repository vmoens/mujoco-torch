...
def _make_joint(joint_type: str, name: str) -> dict[str, str]:
    ...
    if joint_type not in ("free",):
        joint_attr["damping"] = f"{np.random.uniform() * 20:.2f}"
        joint_attr["stiffness"] = f"{np.random.uniform() * 20:.2f}"

    return joint_attr


def _geom_solparams(pair: bool = False, enable_contact: bool = True) -> dict[str, str]:
...
def _make_geom(pos: str, size: float, name: str, enable_contact: bool = True) -> dict[str, str]:
...
def create_mjcf(
    ...
    def make_tree(body: ET.Element, depth: int) -> None:
        ...
        for nj in range(np.random.randint(1, max_stacked_joints + 1)):
            ...
            had_ball_or_free = any([j.get("type") in ("ball", "free") for j in prev_joints])
            if had_ball_or_free:
        ...
        for _ in range(np.random.randint(1, max_geoms_per_body + 1)):
            ...
            geom_attr = _make_geom(pos, size, name=f"geom{n_geoms}", enable_contact=enable_contact)
