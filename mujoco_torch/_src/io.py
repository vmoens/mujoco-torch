...
def _make_data_public_fields(m: Model) -> dict:
    ...
    return {k: torch.zeros(v, dtype=DEFAULT_DTYPE) for k, v in zero_fields.items()}
...
def make_data(m: Model | mujoco.MjModel) -> Data:
...
def put_model(m: mujoco.MjModel, device_target: torch.device | None = None) -> Model:
