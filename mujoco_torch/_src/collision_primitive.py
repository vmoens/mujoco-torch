...
def _plane_sphere(
  ...
  cdist = torch.dot(sphere_pos - plane_pos, plane_normal)
  dist = cdist - radius
...
def plane_capsule(plane: GeomInfo, cap: GeomInfo) -> Contact:
  ...
  y, z = torch.tensor([0.0, 1.0, 0.0]), torch.tensor([0.0, 0.0, 1.0])
  b = torch.where(b_norm < 0.5, torch.where((-0.5 < n[1]) & (n[1] < 0.5), y, z), b)
  frame = torch.tensor([[n, b, torch.linalg.cross(n, b)]])
...
def _sphere_sphere(
  ...
  n = torch.where(dist == 0.0, torch.tensor([1.0, 0.0, 0.0]), n)
