  ...
  def fn(carry, jnt_typs, jnt_pos, jnt_axis, qpos, qpos0, pos, quat):
    ...
    for i, jnt_typ in enumerate(jnt_typs):
      if jnt_typ == JointType.FREE:
        anchor, axis = qpos[qpos_i : qpos_i + 3], torch.tensor(
            [0.0, 0.0, 1.0], dtype=qpos.dtype, device=qpos.device
        )
      else:
...
def com_pos(m: Model, d: Data) -> Data:
  ...
  subtree_com = torch.where(
      cond, d.xipos, torch.vmap(torch.divide)(pos, torch.maximum(mass, torch.tensor(mujoco.mjMINVAL, dtype=mass.dtype)))
  )
...
def dense_m(m: Model, d: Data) -> torch.Tensor:
  ...
  i, j, madr_ij = (
      torch.tensor(x, dtype=torch.int32, device=d.qM.device) for x in (is_, js, madr_ijs)
  )
...
def mul_m(m: Model, d: Data, vec: torch.Tensor) -> torch.Tensor:
  ...
  i, j, madr_ij = (
      torch.tensor(x, dtype=torch.int32, device=vec.device) for x in (is_, js, madr_ijs)
  )
...
def com_vel(m: Model, d: Data) -> Data:
  ...
  def fn(parent, jnt_typs, cdof, qvel):
    ...
    cross_fn = lambda cvel_, cdof_: torch.vmap(math.motion_cross, (None, 0))(cvel_, cdof_) if cdof_.shape[0] > 0 else cdof_
