...
def _ray_capsule(
  ...
  x = torch.where(
      torch.abs(pnt[2] + x * vec[2]) <= size[1], x,
      torch.tensor(torch.inf, dtype=x.dtype, device=x.device)
  )
  ...
  x = torch.where(
      (pnt[2] + x0 * vec[2] >= size[1]) & (x0 < x), x0, x
  )
  x = torch.where(
      (pnt[2] + x1 * vec[2] >= size[1]) & (x1 < x), x1, x
  )
...
def ray(
  ...
  geom_pnts = torch.vmap(
      lambda x, y: x.T @ (pnt - y)
  )(d.geom_xmat, d.geom_xpos)
  ...
  geom_filter_dyn = geom_filter_dyn & (
      (m.geom_matid == -1) | (m.mat_rgba[m.geom_matid, 3] != 0)
  )
