...
def solve(m: Model, d: Data) -> Data:
  ...
  def _linesearch(ctx):
    ...
    quad = torch.stack(
        (0.5 * ctx.Jaref * ctx.Jaref, jv * ctx.Jaref, 0.5 * jv * jv)
    )
    quad = (quad * efc_D).T
    ...
    def ls_body(ls_ctx):
      ...
      lo = torch.utils._pytree.tree_map(
          lambda x, y: torch.where(swap_lo_next, y, x), lo, lo_next
      )
      swap_lo_mid = (mid.deriv_0 < 0) & (lo.deriv_0 < mid.deriv_0)
      lo = torch.utils._pytree.tree_map(
          lambda x, y: torch.where(swap_lo_mid, y, x), lo, mid
      )

      swap_hi_next = (hi.deriv_0 < 0) | (hi.deriv_0 > hi_next.deriv_0)
      hi = torch.utils._pytree.tree_map(
          lambda x, y: torch.where(swap_hi_next, y, x), hi, hi_next
      )
      swap_hi_mid = (mid.deriv_0 > 0) & (hi.deriv_0 > mid.deriv_0)
      hi = torch.utils._pytree.tree_map(
          lambda x, y: torch.where(swap_hi_mid, y, x), hi, mid
      )
