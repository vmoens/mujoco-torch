...
def _get_rand_line_segment(seed=None):
  if seed is not None:
    torch.manual_seed(seed)
  verts = torch.randn(2, 3)
  return verts[0, :], verts[1, :]


def _get_rand_unit(seed: int):
  torch.manual_seed(seed)
  theta = torch.rand(1) * 2 * mathlib.pi
  a = (torch.rand(1) - 0.5) * 2.0
  phi = torch.arccos(a)
  x = torch.sin(phi) * torch.cos(theta)
  y = torch.sin(phi) * torch.sin(theta)
  z = torch.cos(phi)
  return torch.tensor([x, y, z]).squeeze()

def assert_close(a, num):
  torch.testing.assert_close(a, torch.full_like(a, num))
...
class OrthoganalsTest(parameterized.TestCase):
  ...
  def test_orthogonals(self, i):
    ...
    assert_close(torch.linalg.norm(a), 1)
    assert_close(torch.linalg.norm(b), 1)
    assert_close(torch.linalg.norm(c), 1)
    for x in (torch.abs(a.dot(b)), torch.abs(b.dot(c)), torch.abs(a.dot(c))):
...
def _minimize(fn, sample_fn, lb, ub, tol, max_iter=20, seed=42):
  ...
  while prev_val is None or torch.abs(val - prev_val) > tol:
    params = sample_fn(mu, sigma, n, size, lb, ub)
    vals = torch.tensor([fn(p) for p in params])
    if val < vals.min():  # early exit
      return mu
    idx = vals.argsort()
    best_idx = idx[: int(n * 0.05)]
    mu = params[best_idx].mean(0)
    sigma = params[best_idx].std(0) + 1e-10
...
def _closest_segment_to_segment_points(a0, a1, b0, b1):
  dir_a = a1 - a0
  len_a = torch.sqrt(dir_a.dot(dir_a))
  half_len_a = len_a / 2
  dir_a = dir_a / len_a

  dir_b = b1 - b0
  len_b = torch.sqrt(dir_b.dot(dir_b))
  half_len_b = len_b / 2
  dir_b = dir_b / len_b
  ...
  def sample_fn(mu, sigma, n, size, lb, ub):
    params = torch.randn((n, size)).mul_(sigma).add_(mu)
    params = torch.clamp(params, lb, ub)
    return params

  lb = torch.tensor([-half_len_a, -half_len_b])
  ub = torch.tensor([half_len_a, half_len_b])
...
class ClosestSegmentSegmentPointsTest(parameterized.TestCase):
  ...
  def test_closest_segments_points(self):
    a0 = torch.tensor([0.73432405, 0.12372768, 0.20272314])
    a1 = torch.tensor([1.10600128, 0.88555209, 0.65209485])
    ...
    torch.testing.assert_close(best_a, torch.tensor([1.09063, 0.85404, 0.63351]))
    torch.testing.assert_close(best_b, torch.tensor([0.99596, 0.66156, 1.03813]))
  ...
  def test_intersecting_segments(self):
    ...
    best_a, best_b = math.closest_segment_to_segment_points(a0, a1, b0, b1)
    torch.testing.assert_close(best_a, torch.tensor([0.0, 0.0, 0.0]))
    torch.testing.assert_close(best_b, torch.tensor([0.0, 0.0, 0.0]))
  ...
  def test_intersecting_lines(self):
    ...
    best_a, best_b = math.closest_segment_to_segment_points(a0, a1, b0, b1)
    torch.testing.assert_close(best_a, torch.tensor([0.3, 0.3, 0.0]))
    torch.testing.assert_close(best_b, torch.tensor([0.2, 0.4, 0.0]))
  ...
  def test_parallel_segments(self):
    ...
    best_a, best_b = math.closest_segment_to_segment_points(a0, a1, b0, b1)
    torch.testing.assert_close(best_a, torch.tensor([0.0, 0.0, 0.0]))
    torch.testing.assert_close(best_b, torch.tensor([1.0, 0.0, 0.0]))
  ...
  def test_parallel_offset_segments(self):
    ...
    best_a, best_b = math.closest_segment_to_segment_points(a0, a1, b0, b1)
    torch.testing.assert_close(best_a, torch.tensor([0.0, 0.0, 1.0]))
    torch.testing.assert_close(best_b, torch.tensor([1.0, 0.0, 1.0]), msg=f"{best_b}")
  ...
  def test_zero_length_segments(self):
    ...
    best_a, best_b = math.closest_segment_to_segment_points(a0, a1, b0, b1)
    torch.testing.assert_close(best_a, torch.tensor([0.0, 0.0, -1.0]))
    torch.testing.assert_close(best_b, torch.tensor([1.0, 0.0, 0.1]))
  ...
  params = list(zip(math.repeat(torch.arange(10), 10), math.tile(torch.arange(10), 10)))
  ...
  def test_closest_segment_to_segment_points(self, i, j):
    ...
    torch.testing.assert_close(expected_dist, test_dist, atol=1e-2, rtol=1e-2)
