...
def _assert_attr_eq(mujoco_torch_d, mj_d, attr, name, atol):
  if attr == 'efc_address':
    # we do not test efc_address since it gets set in constraint logic
    return
  err_msg = f'mismatch: {attr} in run: {name}'
  mujoco_torch_d, mj_d = getattr(mujoco_torch_d, attr), getattr(mj_d, attr)
  if attr == 'frame':
    mj_d = mj_d.reshape((-1, 3, 3))
  if mujoco_torch_d.shape != mj_d.shape:
    raise AssertionError(f'{attr} shape mismatch: {mujoco_torch_d.shape}, {mj_d.shape}')
  np.testing.assert_allclose(mujoco_torch_d, mj_d, err_msg=err_msg, atol=atol)
...
def _collide(
  ...
  d = mujoco.MjData(m)
  dx = mujoco_torch.device_put(m, d)
...
class CapsuleCollisionTest(parameterized.TestCase):
  ...
  def test_capsule_convex_edge(self):
    ...
    c = tree_map(lambda x: x[None, 0], dx.contact)
    for field in dataclasses.fields(Contact):
...
class ConvexTest(absltest.TestCase):
  ...
  def test_box_plane(self):
    ...
    c = tree_map(lambda x: x[torch.tensor([0, 1])], dx.contact)
    for field in dataclasses.fields(Contact):
...
class NconTest(parameterized.TestCase):
  ...
  def test_ncon_meshes(self):
    ...
    for i in range(m.nnumeric):
      name_ = (
          m.obj_names[m.name_numericadr[i] :].decode('utf-8').split('\x00', 1)[0]
      )
...
class TopKContactTest(absltest.TestCase):
  ...
  def test_top_k_contacts(self):
    ...
    mx_top_k = mujoco_torch.put_model(m)
