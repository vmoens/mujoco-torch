...
class SolverTest(absltest.TestCase):
  ...
  def test_newton(self):
    ...
    dx = torch.compile(mujoco_torch.solve)(mujoco_torch.put_model(m), mujoco_torch.put_data(m, d))
  ...
  def test_no_warmstart(self):
    ...
    dx = torch.compile(mujoco_torch.solve)(mx, mujoco_torch.put_data(m, d))
    nnz = dx.efc_J.any(dim=1)
