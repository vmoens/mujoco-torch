...
class RayTest(absltest.TestCase):
  ...
  def test_ray_nothing(self):
    ...
    mx, dx = mujoco_torch.put_model(m), mujoco_torch.put_data(m, d)

    pnt, vec = torch.tensor([12.146, 1.865, 3.895]), torch.tensor([0, 0, -1.0])
    dist, geomid = torch.compile(mujoco_torch.ray)(mx, dx, pnt, vec)
  ...
  def test_ray_plane(self):
    ...
    pnt, vec = torch.tensor([2, 1, 3.0]), torch.tensor([0.1, 0.2, -1.0])
  ...
  def test_ray_sphere(self):
    ...
    pnt, vec = torch.tensor([0, 0, 1.6]), torch.tensor([0.1, 0.2, -1.0])
    vec /= torch.linalg.norm(vec)
    dist, geomid = torch.compile(mujoco_torch.ray)(mx, dx, pnt, vec)
    _assert_eq(geomid, 1, 'geom_id')
    pnt, vec, unused = np.array(pnt), np.array(vec), np.zeros(1, dtype=np.int32)
  ...
  def test_ray_capsule(self):
    ...
    pnt, vec = torch.tensor([0.5, 1, 1.6]), torch.tensor([0, 0.05, -1.0])
    vec /= torch.linalg.norm(vec)
    dist, geomid = torch.compile(mujoco_torch.ray)(mx, dx, pnt, vec)
    ...
    pnt, vec = torch.tensor([-0.5, 1, 0.05]), torch.tensor([0, 0.05, 1.0])
    vec /= torch.linalg.norm(vec)
    dist, geomid = torch.compile(mujoco_torch.ray)(mx, dx, pnt, vec)
    _assert_eq(geomid, 2, 'geom_id')
    pnt, vec, unused = np.array(pnt), np.array(vec), np.zeros(1, dtype=np.int32)
    ...
    pnt, vec = torch.tensor([0, 1, 0.75]), torch.tensor([1, 0, 0])
    vec /= torch.linalg.norm(vec)
    dist, geomid = torch.compile(mujoco_torch.ray)(mx, dx, pnt, vec)
    _assert_eq(geomid, 2, 'geom_id')
    pnt, vec, unused = np.array(pnt), np.array(vec), np.zeros(1, dtype=np.int32)
  ...
  def test_ray_box(self):
    ...
    pnt, vec = torch.tensor([1, 0, 1.6]), torch.tensor([0, 0.05, -1.0])
    vec /= torch.linalg.norm(vec)
    dist, geomid = torch.compile(mujoco_torch.ray)(mx, dx, pnt, vec)
    ...
    pnt, vec = torch.tensor([1, 0, 0.05]), torch.tensor([0, 0.05, 1.0])
    vec /= torch.linalg.norm(vec)
    dist, geomid = torch.compile(mujoco_torch.ray)(mx, dx, pnt, vec)
    _assert_eq(geomid, 3, 'geom_id')
    pnt, vec, unused = np.array(pnt), np.array(vec), np.zeros(1, dtype=np.int32)
  ...
  def test_ray_mesh(self):
    ...
    pnt, vec = torch.tensor([2.0, 2.0, 2.0]), -torch.tensor([
        1.0,
        1.0,
        1.0,
    ])
    ...
    pnt, vec = torch.tensor([4.0, 2.0, 2.0]), -torch.tensor([
        2.0,
        1.0,
        1.0,
    ])
  ...
  def test_ray_geomgroup(self):
    ...
    mujoco.mj_forward(m, d)
    mx, dx = mujoco_torch.put_model(m), mujoco_torch.put_data(m, d)
    ray_fn = torch.compile(mujoco_torch.ray, static_argnums=(4,))
  ...
  def test_ray_flg_static(self):
    ...
    mujoco.mj_forward(m, d)
    mx, dx = mujoco_torch.put_model(m), mujoco_torch.put_data(m, d)
    ray_fn = torch.compile(mujoco_torch.ray, static_argnames=('flg_static',))
  ...
  def test_ray_bodyexclude(self):
    ...
    mujoco.mj_forward(m, d)
    mx, dx = mujoco_torch.put_model(m), mujoco_torch.put_data(m, d)
    ray_fn = torch.compile(mujoco_torch.ray, static_argnames=('bodyexclude',))
