...
class ConstraintTest(absltest.TestCase):
  ...
  def test_constraints(self):
    ...
    dx = mujoco_torch.make_constraint(mx, dx)
    nnz = dx.efc_J.any(dim=1)
  ...
  def test_disable_refsafe(self):
    ...
    m.opt.disableflags = m.opt.disableflags | mujoco_torch.DisableBit.REFSAFE
  ...
  def test_disable_constraint(self):
    m = test_util.load_test_file('constraints.xml')
    m.opt.disableflags = m.opt.disableflags | mujoco_torch.DisableBit.CONSTRAINT
    ne, nf, nl, nc = mujoco_torch.count_constraints(m)
    self.assertEqual(ne, 0)
    self.assertEqual(nf, 0)
    self.assertEqual(nl, 0)
    self.assertEqual(nc, 0)
    dx = constraint.make_constraint(mujoco_torch.put_model(m), mujoco_torch.make_data(m))
    self.assertEqual(dx.efc_J.shape[0], 0)

  def test_disable_equality(self):
    m = test_util.load_test_file('constraints.xml')
    m.opt.disableflags = m.opt.disableflags | mujoco_torch.DisableBit.EQUALITY
    ne, nf, nl, nc = mujoco_torch.count_constraints(m)
    self.assertEqual(ne, 0)
    self.assertEqual(nf, 0)
  ...
  def test_disable_contact(self):
    m = test_util.load_test_file('constraints.xml')
    m.opt.disableflags = m.opt.disableflags | mujoco_torch.DisableBit.CONTACT
    ne, nf, nl, nc = mujoco_torch.count_constraints(m)
