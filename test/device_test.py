...
class ValidateInputTest(absltest.TestCase):
    def test_solver(self):
        m = mujoco.MjModel.from_xml_string('<mujoco><option solver="PGS"/><worldbody/></mujoco>')
        with self.assertRaises(NotImplementedError):
            mujoco_torch.device_put(m)

    def test_integrator(self):
        m = mujoco.MjModel.from_xml_string('<mujoco><option integrator="implicit"/><worldbody/></mujoco>')
        with self.assertRaises(NotImplementedError):
            mujoco_torch.device_put(m)

    def test_cone(self):
        # Both pyramidal and elliptic cones are now supported; verify they work.
        for cone in ("pyramidal", "elliptic"):
            m = mujoco.MjModel.from_xml_string(f'<mujoco><option cone="{cone}"/><worldbody/></mujoco>')
            mujoco_torch.device_put(m)
    ...
    def test_condim(self):
        ...
        for i in [2, 5]:
            ...
            with self.assertRaises(NotImplementedError):
                mujoco_torch.device_put(m)
        # condim=1, 3, 4 and 6 are accepted
        for i in [1, 3, 4, 6]:
            m.geom_condim[0] = i
            mujoco_torch.device_put(m)
