...
class ModelIOTest(parameterized.TestCase):
  """IO tests for mujoco_torch.Model."""
  ...
  def test_tendon_not_implemented(self):
    with self.assertRaises(NotImplementedError):
      mujoco_torch.put_model(mujoco.MjModel.from_xml_string("""
        <mujoco>
          <worldbody>
            <body>
...
class DataIOTest(parameterized.TestCase):
  """IO tests for mujoco_torch.Data."""

  def test_make_data(self):
    """Test that make_data returns the correct shapes."""

    m = mujoco.MjModel.from_xml_string(_MULTIPLE_CONVEX_OBJECTS)
    d = mujoco_torch.make_data(m)
  ...
  def test_put_data(self):
    ...
    np.testing.assert_allclose(qm, mujoco_torch.full_m(mujoco_torch.put_model(m), dx))
    ...
    d = mujoco.MjData(m)
    mujoco.mj_step(m, d, 2)
    dx_sparse = mujoco_torch.put_data(m, d)
    ...
    d = mujoco.MjData(m)
    mujoco.mj_step(m, d, 2)
    dx_from_dense = mujoco_torch.put_data(m, d)
  ...
  def test_get_data(self):
    ...
    mujoco.mj_step(m, d, 2)
    dx = mujoco_torch.put_data(m, d)
    d_2: mujoco.MjData = mujoco_torch.get_data(m, dx)
  ...
  def test_get_data_batched(self):
    ...
    dx_b = torch.utils._pytree.tree_map(lambda x: torch.stack((x, x + 0.05)), dx)
    ds = mujoco_torch.get_data(m, dx_b)
    self.assertLen(ds, 2)
  ...
  def test_get_data_into(self):
    ...
    dx = mujoco_torch.put_data(m, d)
    d_2 = mujoco.MjData(m)
    mujoco_torch.get_data_into(d_2, m, dx)
  ...
  def test_make_matches_put(self):
    ...
    step_fn_jit = torch.compile(step_fn).lower(dx).compile()

    # placing an MjData onto device should yield the same treedef mujoco_torch.Data as
    # calling make_data.  they should be interchangeable for jax functions:
    step_fn_jit(mujoco_torch.make_data(m))
